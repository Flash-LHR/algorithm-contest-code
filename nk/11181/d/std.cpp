/*************************************************************************
  ...
  > Created Time: 2021-11-12 21:05:06
 ************************************************************************/
...
#endif
	int n; cin >> n;
	rep (i, 3) fep (j, 1, n + 1) cin >> a[i][j], a[i][j] += a[i][j - 1];
	tree<pair<ll, int>, null_type, greater<>, rb_tree_tag, tree_order_statistics_node_update>
		s;
	fep (i, 1, n + 1) q[i] = a[1][i] - a[2][i - 1], s.insert(mkp(q[i], i));
	ll ans{0};
	fep (i, 1, n + 1) {
		ll val = a[1][i - 1] - a[0][i] - a[2][n];
		ans = (ans + s.order_of_key(mkp(val, -1))) % mod;
		s.erase(mkp(q[i], i));
	}
	cout << ans << endl;
